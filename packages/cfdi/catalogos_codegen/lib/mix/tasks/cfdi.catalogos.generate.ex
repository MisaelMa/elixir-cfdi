defmodule Mix.Tasks.Cfdi.Catalogos.Generate do
  use Mix.Task
  ...
  ## Uso
      ...
      mix cfdi.catalogos.generate --xsd path/to/catCFDI.xsd --xlsx path/to/catCFDI.xlsx
      mix cfdi.catalogos.generate --xlsx-url https://...catCFDI_V_4_DDMMYYYY.xls
  ...
  ## Opciones
    ...
    * `--output PATH` — directorio de salida (default: `packages/cfdi/catalogos/lib/cfdi/catalogos`)
  ...
  def run(args) do
    ...
    output_dir = Keyword.get(opts, :output, "packages/cfdi/catalogos/lib/cfdi/catalogos")
    ...
    case Cfdi.Catalogos.Codegen.generate(codegen_opts) do
      {:ok, written_paths} ->
  ...
  defp filter_specs(nil) do
    Cfdi.Catalogos.Codegen.Catalogs.specs()
  end
