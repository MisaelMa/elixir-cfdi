defmodule Cfdi.Catalogos.Codegen do
  @moduledoc """
  Orquestador del pipeline de codegen para cfdi_catalogos.
  ...
  ## Opciones
    ...
    * `:output_dir` — directorio de salida (default: `packages/cfdi/catalogos/lib/cfdi/catalogos`)
  ...
  alias Cfdi.Catalogos.Codegen.CrossValidator
  alias Cfdi.Catalogos.Codegen.Overrides
  alias Cfdi.Catalogos.Codegen.Parsers.Xlsx
  ...
  @default_output_dir "packages/cfdi/catalogos/lib/cfdi/catalogos"
  ...
  defp default_overrides_dir do
    :code.priv_dir(:cfdi_catalogos_codegen)
    |> List.to_string()
    |> Path.join("overrides")
  end
  ...
  defp do_generate_one(spec, xsd_map, xlsx_path, output_dir, overrides_dir) do
    ...
    with {:ok, raw_rows} <- Xlsx.read_sheet(xlsx_path, sheet_name),
         ...
         {:ok, source} <-
           Cfdi.Catalogos.Codegen.Renderer.render(%{spec: spec, entries: entries_with_atoms}) do
