defmodule Sat.Certificados.Ocsp do
  @moduledoc """
  ...
  defp parse_response_status_der(der) do
    ...
    case Sat.Certificados.Asn1.next(der) do
      {{_, _, 0x10}, body, _} ->
        case Sat.Certificados.Asn1.next(body) do
  ...
  defp extract_basic_response(der) do
    ...
    with {{_, _, 0x10}, body, _} <- Sat.Certificados.Asn1.next(der),
         {{_, _, 0x0A}, _status, after_status} <- Sat.Certificados.Asn1.next(body),
         {{_class, _form, 0x00}, ctx_body, _} <- Sat.Certificados.Asn1.next(after_status),
         {{_, _, 0x10}, rb_body, _} <- Sat.Certificados.Asn1.next(ctx_body),
         {{_, _, 0x06}, _oid, after_oid} <- Sat.Certificados.Asn1.next(rb_body),
         {{_, _, 0x04}, basic_der, _} <- Sat.Certificados.Asn1.next(after_oid) do
      {:ok, basic_der}
    else
  ...
  defp parse_certificate_status_der(basic_der) do
    ...
    with {{_, _, 0x10}, body, _} <- Sat.Certificados.Asn1.next(basic_der),
         {{_, _, 0x10}, tbs, _} <- Sat.Certificados.Asn1.next(body),
  ...
  defp walk_children(bin) do
    case Sat.Certificados.Asn1.next(bin) do
  ...
  defp extract_cert_status(single_body) do
    # SingleResponse ::= SEQUENCE { certID, certStatus, thisUpdate, ... }
    with {{_, _, 0x10}, _cert_id, after_cert_id} <- Sat.Certificados.Asn1.next(single_body),
         {tag, status_body, _} <- Sat.Certificados.Asn1.next(after_cert_id) do
      case tag do
  ...
  defp parse_revoked(revoked_body) do
    ...
    case Sat.Certificados.Asn1.next(revoked_body) do
      {{_, _, 0x18}, time_str, _} ->
  ...
  defp verify_response_signature(%Certificate{} = ocsp_cert, basic_der) do
    ...
    with {{_, _, 0x10}, body, _} <- Sat.Certificados.Asn1.next(basic_der),
         ...
         {{_, _, 0x10}, sig_alg_body, after_sig_alg} <- Sat.Certificados.Asn1.next(after_tbs),
         {{_, _, 0x06}, oid_bin, _} <- Sat.Certificados.Asn1.next(sig_alg_body),
         {{_, _, 0x03}, sig_bits, _} <- Sat.Certificados.Asn1.next(after_sig_alg) do
  ...
  defp encode_issuer_name(subject) do
    ...
    with {{_, _, 0x10}, body, _} <- Sat.Certificados.Asn1.next(der),
         {{_, _, 0x10}, tbs, _} <- Sat.Certificados.Asn1.next(body) do
  ...
  defp issuer_position(tbs) do
    case Sat.Certificados.Asn1.next(tbs) do
      {{2, _, 0}, _, _} -> 4
      _ -> 3
    end
  end
  ...
  defp extract_issuer_public_key_bits(issuer) do
    ...
    with {{_, _, 0x10}, body, _} <- Sat.Certificados.Asn1.next(der),
      ...
      with {{_, _, 0x10}, spki_body, _} <- Sat.Certificados.Asn1.next(spki_der),
           {{_, _, 0x10}, _alg, after_alg} <- Sat.Certificados.Asn1.next(spki_body),
           {{_, _, 0x03}, bit_string, _} <- Sat.Certificados.Asn1.next(after_alg) do
