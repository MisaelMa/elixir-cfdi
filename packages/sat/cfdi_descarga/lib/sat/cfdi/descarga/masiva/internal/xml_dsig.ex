defmodule Sat.Cfdi.Descarga.Masiva.Internal.XmlDsig do
  @moduledoc false
  ...
  alias Sat.Cfdi.Descarga.Masiva.Internal.X509Info
  ...
  @doc "Hash SHA-1 + Base64 (DigestValue para XMLDSig)."
  @spec sha1_base64(iodata()) :: String.t()
  def sha1_base64(iodata) do
    :crypto.hash(:sha, iodata) |> Base.encode64()
  end

  @doc "Construye `<SignedInfo>` ya canonicalizado sobre un fragmento referenciado por URI (`#id`)."
  ...
  @doc "Firma `SignedInfo` con la llave privada (RSA-SHA1, base64)."
  ...
  @doc "Construye `<KeyInfo>` con `X509IssuerSerial` + `X509Certificate` (DER b64)."
  ...
  @doc "Construye `<KeyInfo>` que apunta a un `BinarySecurityToken` ya incluido en el `Security` header del SOAP."
  ...
  @doc "Construye `<Signature>` completo combinando SignedInfo + SignatureValue + KeyInfo."
  ...
  @doc "Construye `<o:BinarySecurityToken>` con el certificado DER en base64."
