...
Herramienta de desarrollo que **genera automáticamente** los 15 módulos `.ex` de [`sat_catalogos`](../catalogos) a partir de los archivos oficiales del SAT: `catCFDI.xsd` (códigos válidos) y `catCFDI.xlsx` (descripciones, fechas de vigencia, metadata).
...
mix sat.catalogos.generate --output tmp/generated
...
|----------|--------|---------------|
| `c_FormaPago` | `Sat.Catalogos.FormaPago` | `forma_pago.exs` |
| `c_MetodoPago` | `Sat.Catalogos.MetodoPago` | `metodo_pago.exs` |
| `c_TipoDeComprobante` | `Sat.Catalogos.TipoComprobante` | `tipo_comprobante.exs` |
| `c_Impuesto` | `Sat.Catalogos.Impuesto` | `impuesto.exs` |
| `c_UsoCFDI` | `Sat.Catalogos.UsoCFDI` | `uso_cfdi.exs` |
| `c_Exportacion` | `Sat.Catalogos.Exportacion` | `exportacion.exs` |
| `c_Moneda` | `Sat.Catalogos.Moneda` | `moneda.exs` (code-as-atom) |
...
|----------|--------|---------------------------------------|
| `c_Periodicidad` | `Sat.Catalogos.Periodicidad` | — |
| `c_Meses` | `Sat.Catalogos.Meses` | — |
| `c_TipoRelacion` | `Sat.Catalogos.TipoRelacion` | `tipo_relacion.exs` |
| `c_ObjetoImp` | `Sat.Catalogos.ObjetoImp` | — |
| `c_TipoFactor` | `Sat.Catalogos.TipoFactor` | — |
| `c_Pais` | `Sat.Catalogos.Pais` | — |
| `c_Estado` | `Sat.Catalogos.Estado` | `estado.exs` |

### Con extras (1) — variant `:regimen_fiscal`

| Catálogo | Módulo | Extras |
|----------|--------|--------|
| `c_RegimenFiscal` | `Sat.Catalogos.RegimenFiscal` | `persona_fisica`, `persona_moral`, `inicio_vigencia`, `fin_vigencia` |
...
|--------|----------------|
| `Sat.Catalogos.Codegen.Parsers.Xsd` | Lee `catCFDI.xsd` con regex; devuelve `%{simpletype => [code, ...]}` |
| `Sat.Catalogos.Codegen.Parsers.Xlsx` | Abre `.xlsx` con `:zip.unzip/2`, lee `xl/sharedStrings.xml` y `xl/worksheets/sheet*.xml` con SAX handlers de `Saxy`. Devuelve `[[cell, ...]]`. Sin `xlsxir` |
| `Sat.Catalogos.Codegen.Overrides` | `Code.eval_file/1` sobre archivos `.exs` de `priv/overrides/`; devuelve `%{enum_names: %{}, descriptions: %{}}` |
| `Sat.Catalogos.Codegen.AtomNamer` | Normaliza `"Cheque nominativo"` → `:cheque_nominativo` (NFD strip + ASCII filter + snake_case + trim). Solo se usa para auto-derivar átomos cuando un override tiene un código sin `enum_names` (raramente; las catalogos con átomos siempre traen overrides explícitos) |
| `Sat.Catalogos.Codegen.CrossValidator` | XSD ∩ XLSX ∩ overrides → entries. XLSX sin XSD → `:error`. XSD sin XLSX → `deprecated: true`, label desde `descriptions` o `""`. `ending_date` pasada → `deprecated: true` |
| `Sat.Catalogos.Codegen.Catalogs` | Lista estática `Catalogs.specs/0` con los 15 `Spec` (qué hoja leer, qué columna es el label, padding del código, etc.) |
| `Sat.Catalogos.Codegen.Renderer` | Emite código Elixir formateado. 3 variantes: `:with_atoms`, `:strings_only`, `:regimen_fiscal`. El header banner es **idéntico** en todos los archivos generados |
| `Sat.Catalogos.Codegen` | Orchestrator. Loop sobre `specs`, llama parsers/validator/renderer, escribe archivos |
| `Mix.Tasks.Sat.Catalogos.Generate` | CLI. Parsea flags, opcionalmente descarga via `Sat.Recursos`, llama orchestrator |
...
`Sat.Catalogos.Codegen.Catalogs.Spec` define qué leer y cómo renderizar:

```elixir
%Spec{
  simpletype:      "c_FormaPago",                     # nombre en XSD
  module_name:     Sat.Catalogos.FormaPago,           # módulo destino
...
```bash
git diff packages/sat/catalogos/lib/sat/catalogos/
```
...
cd packages/sat/catalogos_codegen && mix test --exclude network

git add packages/files/4.0/catCFDI.xlsx \
        packages/sat/catalogos/lib/sat/catalogos/ \
        packages/sat/catalogos_codegen/priv/overrides/ \
...
1. **Agregar el spec** en `lib/sat/catalogos/codegen/catalogs.ex`:
   ```elixir
   %Spec{
     simpletype: "c_NuevoCatalogo",
     module_name: Sat.Catalogos.NuevoCatalogo,
...
| Mix task | `Mix.Tasks.Sat.Catalogos.Generate.run/1` con flags |
...
- **Output commiteado**: los `.ex` generados viven en git en `packages/sat/catalogos/lib/sat/catalogos/`. NO se generan en compile-time. Esto da reproducibilidad, hace los diffs auditables en PRs y permite a `sat_catalogos` no tener `saxy` como dep.
...
- **No hay shims de compatibilidad**: cuando un cambio rompe la API (ej. `RegimenFiscal.value` int → string), arreglamos los call sites internos en el mismo PR. Es viable porque `sat_catalogos` no está publicado en Hex todavía.
- **`only: :dev, runtime: false`**: este paquete NO se carga en runtime. La advertencia de compilación sobre `Sat.Recursos.SatResources` desde otros paquetes es esperable mientras `sat_catalogos_codegen` no esté en su `deps`.
...
├── lib/
│   ├── sat/catalogos/codegen.ex             ← Sat.Catalogos.Codegen.generate/1
...
│   └── mix/tasks/sat.catalogos.generate.ex   ← Mix.Task
...
└── test/
    ...
    └── sat/catalogos/codegen/...              ← tests por módulo
```
