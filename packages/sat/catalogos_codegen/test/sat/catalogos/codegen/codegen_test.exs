defmodule Sat.Catalogos.CodegenTest do
  use ExUnit.Case, async: true

  alias Sat.Catalogos.Codegen
  ...
  describe "normalize_code/2" do
    test "integer 1 with pad 2 becomes '01'" do
      assert Sat.Catalogos.Codegen.normalize_code(1, 2) == "01"
    end

    test "integer 12 with pad 2 stays '12' (already 2 chars)" do
      assert Sat.Catalogos.Codegen.normalize_code(12, 2) == "12"
    end

    test "integer 1 with pad 3 becomes '001'" do
      assert Sat.Catalogos.Codegen.normalize_code(1, 3) == "001"
    end

    test "integer 1 with pad 0 becomes '1' (no padding)" do
      assert Sat.Catalogos.Codegen.normalize_code(1, 0) == "1"
    end

    test "string '01' with pad 2 stays '01' (strings pass through untouched)" do
      assert Sat.Catalogos.Codegen.normalize_code("01", 2) == "01"
    end

    test "string 'PUE' with pad 0 stays 'PUE'" do
      assert Sat.Catalogos.Codegen.normalize_code("PUE", 0) == "PUE"
    end
    ...
    test "string '601' with pad 0 stays '601'" do
      assert Sat.Catalogos.Codegen.normalize_code("601", 0) == "601"
    end

    test "nil with pad 0 becomes empty string" do
      assert Sat.Catalogos.Codegen.normalize_code(nil, 0) == ""
    end
  end
  ...
  describe "parse_date/1 — integer serial dates" do
    test "serial string '44562' resolves to ~D[2022-01-01]" do
      assert Sat.Catalogos.Codegen.parse_date("44562") == ~D[2022-01-01]
    end
    ...
    test "serial string '1' resolves to ~D[1899-12-31]" do
      assert Sat.Catalogos.Codegen.parse_date("1") == ~D[1899-12-31]
    end

    test "existing DD/MM/YYYY format still works" do
      assert Sat.Catalogos.Codegen.parse_date("01/01/2022") == ~D[2022-01-01]
    end

    test "nil returns nil" do
      assert Sat.Catalogos.Codegen.parse_date(nil) == nil
    end

    test "empty string returns nil" do
      assert Sat.Catalogos.Codegen.parse_date("") == nil
    end

    test "unreasonable serial (too large) returns nil" do
      # year 2100+ is considered unreasonable
      assert Sat.Catalogos.Codegen.parse_date("999999") == nil
    end
  end
  ...
  describe "parse_xlsx_rows/2 — label_column" do
    ...
    test "label_column: 2 extracts col C as label, NOT col B" do
      ...
      unless File.exists?(xlsx_path) do
        ...
        estado_spec = %Spec{
          simpletype: "c_Estado",
          module_name: Sat.Catalogos.Estado,
          file_name: "estado.ex",
    ...
    test "label_column: 0 uses the code itself as label (for c_TipoFactor shape)" do
      ...
      unless File.exists?(xlsx_path) do
        ...
        tipo_factor_spec = %Spec{
          ...
          module_name: Sat.Catalogos.TipoFactor,
    ...
    test "a deprecated code with override description gets that label instead of empty string" do
      ...
      unless File.exists?(xlsx_path) do
        ...
        tipo_relacion_spec = %Spec{
          ...
          module_name: Sat.Catalogos.TipoRelacion,
    ...
    test "regimen_fiscal deprecated codes 609, 628, 629, 630 get canonical descriptions from override" do
      ...
      unless File.exists?(xlsx_path) do
        ...
        regimen_spec = %Spec{
          ...
          module_name: Sat.Catalogos.RegimenFiscal,
  ...
  defp spec_c_a do
    %Spec{
      simpletype: "c_A",
      module_name: Sat.Catalogos.TestCatalogA,
  ...
  defp spec_c_b do
    %Spec{
      simpletype: "c_B",
      module_name: Sat.Catalogos.TestCatalogB,
  ...
  describe "generate/1 — happy path" do
    test "writes one .ex file when catalogs: filtered to c_A" do
      ...
      assert content =~ "defmodule Sat.Catalogos.TestCatalogA"
    end
