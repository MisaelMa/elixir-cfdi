defmodule Clir.Openssl.Pkcs8 do
  ...
  def to_pem(der_binary, password \\ nil) when is_binary(der_binary) do
    ...
    with {:ok, decoded} <- decode_private_der(der_binary, password),
         {:ok, pem} <- safe_encode_decoded_to_pem(decoded) do
      {:ok, pem}
    else
      _ -> openssl_pkcs8_to_pem(der_binary, password)
    end
  end

  defp safe_encode_decoded_to_pem(decoded) when is_tuple(decoded) and tuple_size(decoded) > 0 do
    case elem(decoded, 0) do
      :RSAPrivateKey -> try_encode(:RSAPrivateKey, decoded)
      :PrivateKeyInfo -> try_encode(:PrivateKeyInfo, decoded)
      :ECPrivateKey -> try_encode(:ECPrivateKey, decoded)
      :DSAPrivateKey -> try_encode(:DSAPrivateKey, decoded)
      _ -> {:error, :unsupported_decoded}
    end
  end

  defp safe_encode_decoded_to_pem(_), do: {:error, :unsupported_decoded}

  defp try_encode(type, term) do
    der = :public_key.der_encode(type, term)
    {:ok, :public_key.pem_encode([{type, der, :not_encrypted}])}
  rescue
